...
// recovered = -2
// deceased = -1
using Host = std::tuple<short,short>;

class Pathogen {
public:
        Pathogen(std::string name = "Ebola", double pE = 0.005, double pD = 0.5, short minE = 2, short kE = 9, short kI = 16)
                : name(name), pcatch(pE), pdie(pD), edist(1.0/(kE-minE+1)), kI(kI), minE(minE) {}
        ...
        bool infectious  (Host const& h) { return std::get<1>(h) < std::get<0>(h) && std::get<0>(h) <= (std::get<1>(h) + kI); }
...
init( int h, int w, int kE, int kI )
{
    ...
    glVertexAttribIPointer( sloc, 2, GL_SHORT, 0, BUFFER_OFFSET(0) );
...
void resetMap()
{
	m = { m.size(), std::vector<Host>( m[0].size(), std::make_pair( 0, 0 ) ) };
...
main( int argc, char **argv )
{
	...
	m = { N, std::vector<Host>( N, std::make_pair( 0, 0 ) ) };

	ebola = { "Ebola-like", PT, PD, ME, KE, KI };
